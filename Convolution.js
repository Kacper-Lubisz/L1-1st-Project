...
class Convolution {
    constructor(kernel, kernelShape, wrappingMode) {
        ...
        if (kernelShape.length < 2) {
            throw "Kernel size is invalid, number of dimensions must be >= 2";
        } else if (kernelShape.length > 4) {
            throw "Kernel size is invalid, number of dimensions must be <= 4";
        } else { // number of dimensions is valid, now validate each dimension
            const colorDim = kernelShape[2] || 1;
            const forceDim = kernelShape[3] || 1;

            if (!(colorDim === 1 || colorDim === 3 || colorDim === 4)) {
                throw "Number of color dimensions " + colorDim + " is invalid, color can only be 1, 3 or 4 dimensional"
            }
            if (!(forceDim === 1 || forceDim === 2)) {
                throw "Number of force dimensions " + forceDim + " is invalid, it must be either a scalar or vector (1 or 2)"
            }
        }
